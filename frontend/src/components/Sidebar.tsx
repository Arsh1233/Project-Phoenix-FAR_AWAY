...
interface Props {
  activePanel: 'overview' | 'crypto' | 'edge' | 'leak' | 'blockchain';
  setActivePanel: (panel: 'overview' | 'crypto' | 'edge' | 'leak' | 'blockchain') => void;
}
  ...
  return (
    <aside className="w-64 bg-white border-r border-surface-200 flex flex-col h-full shrink-0">
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        <button 
          className={`nav-item ${activePanel === 'edge' ? 'active' : ''}`}
